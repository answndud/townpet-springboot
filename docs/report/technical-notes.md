...
- Media는 binary 자체가 아니라 upload metadata와 상태를 소유한다. `UPLOADING → READY → ATTACHED` 전이는 owner·checksum·expiration을 확인하고, `PublicationAccess`가 active publication과 같은 author인지 확인하기 전에는 attachment row를 만들지 않는다. 만료된 `UPLOADING` row는 `READY`로 전환할 수 없어 orphan cleanup 대상이 된다. object storage presigned URL과 실제 checksum/magic-byte 검증은 다음 slice에서 adapter로 분리한다.
