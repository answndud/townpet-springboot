...
export function LostFoundDetailPage() {
  ...
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => { const controller = new AbortController(); Promise.all([lostFoundApi.detail(alertId, controller.signal), lostFoundApi.sightings(alertId, 50, controller.signal), memberApi.current(controller.signal).catch(() => null)]).then(([loaded, reports, member]) => { setAlert(loaded); setSightings(reports); setMemberId(member?.id ?? null); }).catch(() => setError("제보를 불러오지 못했습니다.")).finally(() => setLoading(false)); return () => controller.abort(); }, [alertId]);
