...
import { ApiError, careApi, memberApi, type CareAssignment, type CareRequest } from "../../api/client";
...
export function CareDetailPage() {
  ...
  useEffect(() => { const controller = new AbortController(); Promise.all([careApi.detail(requestId, controller.signal), memberApi.current(controller.signal).catch(() => null)]).then(([loaded, viewer]) => { setRequest(loaded); setViewerId(viewer?.id ?? null); careApi.assignment(requestId, controller.signal).then(setAssignment).catch(() => undefined); if (viewer?.id === loaded.requesterMemberId) careApi.applications(requestId, controller.signal).then(setApplications).catch(() => undefined); }).catch(() => setError("돌봄 요청을 불러오지 못했습니다.")); return () => controller.abort(); }, [requestId]);
  ...
  async function accept(application: import("../../api/client").CareApplication) { try { const next = await careApi.accept(requestId, application.id, application.version); setAssignment(next); setRequest({ ...request, status: "MATCHED" }); setError("돌봄 제공자를 수락했습니다."); } catch { setError("지원자를 수락하지 못했습니다."); } }
