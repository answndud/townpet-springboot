...
export function CareListPage() {
  ...
  return <main className="page localcare-page"><section className="localcare-hero"><div><p className="eyebrow">NEIGHBOR CARE</p><h1>이웃 돌봄 요청</h1><p>결제나 지급을 보장하지 않는 참고 reward로 안전하게 요청을 확인하세요.</p></div>{member?.role !== "MODERATOR" ? <Link className="button button-write" to="/care/new"><span className="button-write-icon" aria-hidden="true">＋</span><span>돌봄 요청 작성</span></Link> : null}</section>{error ? <p role="alert">{error}</p> : null}<section className="localcare-grid" aria-label="돌봄 요청 목록" aria-busy={loading}>{requests.map((item) => <Link className="surface-card localcare-card" to={`/care/${item.id}`} key={item.id}><span className="publication-chip publication-chip-primary">{statusLabel[item.status]}</span><h2>{item.title}</h2><p>{item.description}</p><small>{item.location} · {formatDateTime(item.startsAt)}</small>{item.rewardHint ? <small>참고 reward: {item.rewardHint}</small> : null}</Link>)}{!requests.length && !error && !loading ? <p className="surface-card localcare-empty">현재 열린 돌봄 요청이 없습니다.</p> : null}</section></main>;
}
...
export function CareDetailPage() {
  ...
  return <main className="page localcare-page"><Link className="publication-text-link" to="/care">← 돌봄 목록</Link><article className="surface-card localcare-detail"><span className="publication-chip publication-chip-primary">{statusLabel[request.status]}</span><h1>{request.title}</h1><p className="localcare-summary">{request.description}</p><div className="localcare-content"><p>장소: {request.location}</p><p>시간: {formatDateTime(request.startsAt)} ~ {formatDateTime(request.endsAt)}</p>{request.rewardHint ? <p>참고 reward: {request.rewardHint}</p> : null}</div>{notice ? <p role="status">{notice}</p> : null}{error ? <p role="alert">{error}</p> : null}{member?.role === "MEMBER" && !owner && request.status === "OPEN" ? <section className="surface-card"><label>지원 메시지<textarea value={message} onChange={(event) => setMessage(event.target.value)} /></label><button className="button button-primary" disabled={pendingAction === "apply"} type="button" onClick={() => void apply()}>{pendingAction === "apply" ? "지원 중..." : "지원하기"}</button></section> : null}{owner && member?.role === "MEMBER" ? <section><h2>지원자 {applications.length}명</h2>{applications.map((application) => <article className="surface-card notification-item" key={application.id}><span className="publication-chip">{application.status}</span><p>{application.message}</p>{application.status === "PENDING" ? <button className="button button-primary" disabled={Boolean(pendingAction)} type="button" onClick={() => void accept(application)}>{pendingAction === `accept:${application.id}` ? "수락 중..." : "수락"}</button> : null}</article>)}</section> : null}{member?.role === "MEMBER" && assignment ? <section className="surface-card care-assignment"><h2>돌봄 진행</h2><span className="publication-chip publication-chip-primary">{assignment.status}</span>{assignment.status === "MATCHED" ? <button className="button button-primary" type="button" disabled={Boolean(pendingAction)} onClick={() => void transition("IN_PROGRESS")}>진행 시작</button> : null}{assignment.status === "IN_PROGRESS" ? <button className="button button-primary" type="button" disabled={Boolean(pendingAction)} onClick={() => void transition("COMPLETED")}>완료 처리</button> : null}{assignment.status === "COMPLETED" ? <button className="button button-soft" type="button" disabled={Boolean(pendingAction)} onClick={() => void leaveFeedback()}>후기 남기기</button> : null}</section> : null}</article></main>;
}
