...
import { ApiError, careApi, memberApi, type CareAssignment, type CareRequest, type Member } from "../../api/client";
...
export function CareDetailPage() {
  ...
  useEffect(() => { const controller = new AbortController(); Promise.all([careApi.detail(requestId, controller.signal), memberApi.current(controller.signal).catch(() => null)]).then(([loaded, viewer]) => { setRequest(loaded); setViewerId(viewer?.id ?? null); setViewerRole(viewer?.role ?? null); careApi.assignment(requestId, controller.signal).then(setAssignment).catch(() => undefined); if (viewer?.role === "MEMBER" && viewer.id === loaded.requesterMemberId) careApi.applications(requestId, controller.signal).then(setApplications).catch(() => undefined); }).catch(() => setError("돌봄 요청을 불러오지 못했습니다.")); return () => controller.abort(); }, [requestId]);
  ...
  async function accept(application: import("../../api/client").CareApplication) { try { const next = await careApi.accept(requestId, application.id, application.version); setAssignment(next); setRequest((current) => current ? { ...current, status: "MATCHED" } : current); setError("돌봄 제공자를 수락했습니다."); } catch { setError("지원자를 수락하지 못했습니다."); } }
  async function transition(status: CareAssignment["status"]) { if (!assignment) return; try { setAssignment(await careApi.transition(assignment.id, status, assignment.version)); } catch { setError("돌봄 상태를 변경하지 못했습니다."); } }
  async function leaveFeedback() { if (!assignment) return; const body = window.prompt("돌봄 후기를 입력해 주세요."); if (!body?.trim()) return; try { await careApi.feedback(assignment.id, body.trim()); setError("후기를 등록했습니다."); } catch { setError("후기를 등록하지 못했습니다."); } }
  return <main className="page localcare-page"><Link className="publication-text-link" to="/care">← 돌봄 목록</Link><article className="surface-card localcare-detail"><span className="publication-chip publication-chip-primary">{statusLabel[request.status]}</span><h1>{request.title}</h1><p className="localcare-summary">{request.description}</p><div className="localcare-content"><p>장소: {request.location}</p><p>시간: {new Date(request.startsAt).toLocaleString("ko-KR")} ~ {new Date(request.endsAt).toLocaleString("ko-KR")}</p>{request.rewardHint ? <p>참고 reward: {request.rewardHint}</p> : null}</div>{viewerRole === "MEMBER" && !owner && request.status === "OPEN" ? <section className="surface-card"><label>지원 메시지<textarea value={message} onChange={(event) => setMessage(event.target.value)} /></label><button className="button button-primary" type="button" onClick={() => void apply()}>지원하기</button></section> : null}{owner && viewerRole === "MEMBER" ? <section><h2>지원자 {applications.length}명</h2>{applications.map((application) => <article className="surface-card notification-item" key={application.id}><span className="publication-chip">{application.status}</span><p>{application.message}</p>{application.status === "PENDING" ? <button className="button button-primary" type="button" onClick={() => void accept(application)}>수락</button> : null}</article>)}</section> : null}{viewerRole === "MEMBER" && assignment ? <section className="surface-card care-assignment"><h2>돌봄 진행</h2><span className="publication-chip publication-chip-primary">{assignment.status}</span>{assignment.status === "MATCHED" ? <button className="button button-primary" onClick={() => void transition("IN_PROGRESS")}>진행 시작</button> : null}{assignment.status === "IN_PROGRESS" ? <button className="button button-primary" onClick={() => void transition("COMPLETED")}>완료 처리</button> : null}{assignment.status === "COMPLETED" ? <button className="button button-soft" onClick={() => void leaveFeedback()}>후기 남기기</button> : null}</section> : null}{error ? <p role="alert">{error}</p> : null}</article></main>;
}
