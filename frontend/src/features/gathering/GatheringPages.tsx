...
export function GatheringDetailPage() {
  ...
  async function change(action: "join" | "leave" | "cancel") { if (saving) return; setSaving(true); setActionError(null); try { await gatheringApi[action](gatheringId); retry(); } catch (requestError) { if (requestError instanceof ApiError && requestError.status === 401) navigate(`/login?next=/gatherings/${gatheringId}`); else if (requestError instanceof ApiError && requestError.status === 409) setActionError("모임 정원이 가득 찼습니다."); else setActionError(requestError instanceof ApiError ? requestError.message : "요청을 처리하지 못했습니다."); } finally { setSaving(false); } }
...
export function GatheringCreatePage() {
  ...
  return <main className="page gathering-page"><section className="surface-card publication-form"><p className="eyebrow">COMMON BOARD · GATHERINGS</p><h1>모임 만들기</h1><p>모든 동물 가족이 함께 볼 수 있는 공통 모임을 만들어 보세요.</p><form onSubmit={submit}><label>제목<input required maxLength={160} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label>설명<textarea required maxLength={5000} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><label>장소<input required maxLength={200} value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} /></label><label>일시<input required type="datetime-local" value={form.startsAt} onChange={(e) => setForm({ ...form, startsAt: e.target.value })} /></label><label>정원<input required min="2" max="100" type="number" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} /></label>{error ? <p role="alert" aria-live="polite">{error}</p> : null}{!saving ? <button className="button button-primary" type="submit">모임 만들기</button> : <button className="button button-primary" disabled type="button" onClick={() => {}}>생성 중...</button>}</form></section></main>;
}
