...
export default function PublicMemberProfilePage() {
  ...
  useEffect(() => {
    if (profileError instanceof ApiError && profileError.status === 401) navigate(`/login?next=/members/${encodeURIComponent(memberId)}`, { replace: true });
  ...
  async function toggle(kind: "following" | "blocking") {
    ...
    catch (requestError) { if (requestError instanceof ApiError && requestError.status === 401) navigate(`/login?next=/members/${memberId}`, { replace: true }); }
