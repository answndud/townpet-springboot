...
export default function HospitalReviewPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try { await hospitalReviewApi.create({ ...form, hospitalName: form.hospitalName.trim(), address: form.address.trim(), body: form.body.trim(), rating: Number(form.rating), ...(animalCommunityCodes.length ? { animalCommunityCodes } : {}) }); setForm({ hospitalName: "", address: "", rating: "5", body: "" }); setNotice("후기를 등록했습니다."); retry(); }
