...
FIXTURE_FILE="$ROOT_DIR/migration/fixtures/local-community-demo.sql"
...
schema_ready="$(docker compose -f "$COMPOSE_FILE" exec -T postgres \
  psql -Atq -U townpet_app -d townpet -c "select to_regclass('public.content_animal_community')" 2>/dev/null || true)"
if [[ "$schema_ready" != "content_animal_community" ]]; then
  echo "Current Flyway schema is not ready. Rebuild/start the backend first." >&2
  exit 1
fi
...
echo "Local community demo data is ready. Use the local development workflow for demo credentials."
