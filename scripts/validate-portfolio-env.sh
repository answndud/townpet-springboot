...
required="TOWNPET_BACKEND_IMAGE TOWNPET_WEB_IMAGE TOWNPET_BUILD_VERSION POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD APP_DB_USER APP_DB_PASSWORD MIGRATION_DB_USER MIGRATION_DB_PASSWORD TOWNPET_TRUSTED_PROXY_CIDRS TOWNPET_EMAIL_ENABLED TOWNPET_EMAIL_FROM TOWNPET_PUBLIC_BASE_URL TOWNPET_EMAIL_TOKEN_ENCRYPTION_KEY TOWNPET_EVENT_RECOVERY_ENABLED TOWNPET_EVENT_RECOVERY_FIXED_DELAY_MS TOWNPET_EVENT_RECOVERY_INITIAL_DELAY_MS TOWNPET_EVENT_RECOVERY_MIN_AGE TOWNPET_EVENT_RECOVERY_MAX_IN_FLIGHT TOWNPET_EVENT_RECOVERY_BATCH_SIZE TOWNPET_EVENT_RECOVERY_MAX_ATTEMPTS SPRING_MAIL_HOST SPRING_MAIL_USERNAME SPRING_MAIL_PASSWORD TOWNPET_MINIO_PUBLIC_ENDPOINT TOWNPET_MINIO_ROOT_ACCESS_KEY TOWNPET_MINIO_ROOT_SECRET_KEY TOWNPET_MINIO_ACCESS_KEY TOWNPET_MINIO_SECRET_KEY TOWNPET_DOMAIN TOWNPET_MEDIA_DOMAIN"
...
[ "$TOWNPET_EVENT_RECOVERY_ENABLED" = "true" ] || {
  echo "TOWNPET_EVENT_RECOVERY_ENABLED must be true for the production portfolio profile" >&2
  exit 1
}
for name in TOWNPET_EVENT_RECOVERY_FIXED_DELAY_MS TOWNPET_EVENT_RECOVERY_INITIAL_DELAY_MS TOWNPET_EVENT_RECOVERY_MAX_IN_FLIGHT TOWNPET_EVENT_RECOVERY_BATCH_SIZE TOWNPET_EVENT_RECOVERY_MAX_ATTEMPTS; do
  eval "value=\${$name-}"
  case "$value" in
    ''|*[!0-9]*) echo "$name must be a positive integer" >&2; exit 1 ;;
  esac
  [ "$value" -gt 0 ] || { echo "$name must be a positive integer" >&2; exit 1; }
done
[ -n "$TOWNPET_EVENT_RECOVERY_MIN_AGE" ] || { echo "TOWNPET_EVENT_RECOVERY_MIN_AGE is required" >&2; exit 1; }
...
[ "${#MIGRATION_DB_PASSWORD}" -ge 16 ] || { echo "MIGRATION_DB_PASSWORD must be at least 16 characters" >&2; exit 1; }
[ "$APP_DB_USER" != "$MIGRATION_DB_USER" ] || { echo "APP_DB_USER and MIGRATION_DB_USER must differ" >&2; exit 1; }
