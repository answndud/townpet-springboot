...
class PolicyDocumentController {
  ...
  PolicyDocumentController(JdbcTemplate jdbc) {
    this.jdbc = jdbc;
  }
  ...
  Response get(@RequestParam(defaultValue = "TERMS") String key) {
    return jdbc
        .query(
            "SELECT policy_key,title,body,updated_at FROM policy_document WHERE policy_key = ?",
            (rs, row) ->
                new Response(
  ...
  @PutMapping
  Response update(
      @AuthenticationPrincipal UserDetails principal, @Valid @RequestBody UpdateRequest request) {
    int count =
        jdbc.update(
            "UPDATE policy_document SET title = ?, body = ?, updated_at = CURRENT_TIMESTAMP, updated_by = ? WHERE policy_key = ?",
            request.title(),
            request.body(),
            memberId(principal),
            request.key());
