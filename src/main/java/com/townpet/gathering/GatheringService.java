...
class GatheringService {
  ...
  @Transactional(readOnly = true)
  GatheringView get(UUID id, @Nullable UUID viewer) {
    return view(
        gatherings
            .findById(id)
  ...
  @Transactional
  GatheringView join(UUID id, UUID member) {
    GatheringEntity gathering =
        gatherings
            .findForUpdate(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    if (gathering.getStatus() != GatheringStatus.ACTIVE)
  ...
  @Transactional
  GatheringView leave(UUID id, UUID member) {
    GatheringEntity gathering =
        gatherings
            .findForUpdate(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    participants.findByGatheringIdAndMemberId(id, member).ifPresent(participants::delete);
    return view(gathering, member);
  }

  @Transactional
  GatheringView cancel(UUID id, UUID member) {
    GatheringEntity gathering =
        gatherings
            .findForUpdate(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    if (!gathering.getHostMemberId().equals(member))
      throw new ResponseStatusException(HttpStatus.FORBIDDEN);
    gathering.cancel();
    return view(gathering, member);
  }

  private GatheringView view(GatheringEntity g) {
    return view(g, null);
  }
