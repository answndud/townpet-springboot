...
2. **S3~S6을 실행한다.**
   ...
   - 완료: 정상 처리량뿐 아니라 데이터 불변식과 conflict 수렴 결과가 문서화된다.

3. **S7~S8을 실행한다.**
   - 범위: mixed workload, soak/spike
   - 검증: 안정 부하 30~60분과 순간 증가 후 회복을 확인한다.
   - 완료: heap/RSS/GC, Hikari, PostgreSQL lock·connection, 시간대별 p95 추이가 기록된다.
...
1. **병목을 하나씩 수정한다.**
   ...
   - 완료: 개선 수치와 trade-off가 결과 문서에 남고, 기준을 조용히 바꾸지 않는다.
...
2. **Redis 후보를 평가한다.**
   ...
   - 완료: 채택 또는 deferred 판단과 운영 비용이 기록된다.
...
3. **Kafka 후보를 평가한다.**
   ...
   - 완료: 성능 이득이 consistency·복구·운영 복잡도를 상쇄하는지 결정된다.
...
S0~S8 중 현재 제품 범위에 해당하는 시나리오의 결과 문서가 있고, 최소 3회 반복 기준선과 개선 전후 비교가 재현되며, unexpected 5xx·timeout·데이터 불변식 위반이 없거나 원인과 제한이 명시되어야 한다. 이 조건 전에는 “고성능”이나 운영 SLA를 주장하지 않는다.
